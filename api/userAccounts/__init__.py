...
app = FastAPI(title="User-Accounts-Holdings API", version=1.0, root_path=api_root_path)
...
@app.get("/")
def get_root():
    return {"message": "Root of User-Accounts-Holdings API"}
...
@app.get("/aws/{qp}")
def get_aws_scope(qp: str, request: Request):
    logger.info(request)
    logger.info(qp)
    logger.info(request.scope["aws.context"])
